...
def eca_block(inputs, b=1, gamma=2):
    channels = K.int_shape(inputs)[-1]  # 获取实际输入通道数
...
def ecaResUNet(num_features, num_outputs, args):
    """构建 ResUNet 模型，加入 eca块"""
...
def ecaResUNet_classification(num_features, num_outputs, args):
    """构建 ResUNet 模型，加入 eca 块， 用于classification"""
    inputs = layers.Input(shape=(num_features, 1))
